...
def main():
    ...
    in_hidden_edges = make_weighted_edges(in_nodes, hidden_nodes[:-1], hidden_weights)
    hidden_out_edges = make_weighted_edges(hidden_nodes, out_nodes, out_weights)
...
def make_weighted_edges(in_nodes, out_nodes, weights, normalize=False):
    ...
    for i, in_node in enumerate(in_nodes):
        for j, out_node in enumerate(out_nodes):
            edge_weight = abs(weights[i][j].item())
            edges.append((in_node, out_node, {'weight': edge_weight, 'distance': 1/edge_weight}))
